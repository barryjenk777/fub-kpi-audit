...
def is_within_sms_quiet_hours(tz_name: str = "America/New_York") -> bool:
    """
    Return True if current time is within send-safe hours: 8am-9pm local.

    Project Blue sends via iMessage (data protocol), not carrier SMS, so TCPA's
    strict 8am-9pm window doesn't legally apply — but we use the same window
    intentionally: nobody wants a real-estate text before 8am or after 9pm.
